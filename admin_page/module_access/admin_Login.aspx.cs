...
public partial class admin_page_module_access_admin_Login : System.Web.UI.Page
{
    ...
    protected void btnLogin_ServerClick(object sender, EventArgs e)
    {
        ...
        var viewTaiXe = from tb in db.admin_Users
                           where tb.username_username == userName.ToLower()
                           && tb.username_password == passmd5
                           && tb.username_active == true
                           && tb.groupuser_id == 3
                           select tb;
        if (viewTaiXe.Count() > 0)
        {
            admin_User list = viewTaiXe.Single();
            HttpCookie ck = new HttpCookie("TaiXe");
            string s = ck.Value;
            ck.Value = userName;
            ck.Expires = DateTime.Now.AddDays(365);
            Response.Cookies.Add(ck);
            Response.Redirect("/xac-nhan-tai-xe");
        }
        else
        {
            dem++;
