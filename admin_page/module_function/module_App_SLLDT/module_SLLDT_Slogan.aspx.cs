...
public partial class admin_page_module_function_module_SLLDT_Slogan : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        var getData = from nc in db.tbLandingPage_TungCap_Slogans
                      orderby nc.slogan_id descending
                      select nc;
    ...
    protected void btnChiTiet_Click(object sender, EventArgs e)
    {
        ...
        _id = Convert.ToInt32(grvList.GetRowValues(grvList.FocusedRowIndex, new string[] { "slogan_id" }));
        // đẩy id vào session
        Session["_id"] = _id;
        var getData = (from nc in db.tbLandingPage_TungCap_Slogans
                       where nc.slogan_id == _id
                       select nc
                       ).Single();
        txtTitle.Text = getData.slogan_name;
        image = getData.slogan_image;
        if (getData.slogan_image == null)
            ...
            image = getData.slogan_image;
        ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "Detail", "popupControl.Show();showImg1_1('" + getData.slogan_image + "'); ", true);
        loadData();
    }
    ...
    protected void btnLuu_Click(object sender, EventArgs e)
    {
        ...
        cls_LandingPage_Slogan cls = new cls_LandingPage_Slogan();
    ...
    {
        List<object> selectedKey = grvList.GetSelectedFieldValues(new string[] { "slogan_id" });
        if (selectedKey.Count > 0)
        {
            foreach (var item in selectedKey)
            {
                cls_LandingPage_Slogan cls = new cls_LandingPage_Slogan();
                tbLandingPage_TungCap_Slogan checkImage = (from i in db.tbLandingPage_TungCap_Slogans where i.slogan_id == Convert.ToInt32(item) select i).SingleOrDefault();
