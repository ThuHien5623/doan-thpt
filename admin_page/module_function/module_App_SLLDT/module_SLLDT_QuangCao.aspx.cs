...
public partial class admin_page_module_function_module_SLLDT_QuangCao : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        var getData = from nc in db.tbLandingPage_QuangCaos
                      orderby nc.quangcao_id descending
                      select nc;
    ...
    protected void btnChiTiet_Click(object sender, EventArgs e)
    {
        ...
        _id = Convert.ToInt32(grvList.GetRowValues(grvList.FocusedRowIndex, new string[] { "quangcao_id" }));
        // đẩy id vào session
        Session["_id"] = _id;
        var getData = (from nc in db.tbLandingPage_QuangCaos
                       where nc.quangcao_id == _id
                       select nc
                       ).Single();
        txtTitle.Text = getData.quangcao_price;
        txtLink.Text = getData.quangcao_link;
        image = getData.quangcao_image;
        if (getData.quangcao_image == null)
            ...
            image = getData.quangcao_image;
        ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "Detail", "popupControl.Show();showImg1_1('" + getData.quangcao_image + "'); ", true);
        loadData();
    }
    ...
    protected void btnLuu_Click(object sender, EventArgs e)
    {
        ...
        cls_LandingPage_QuangCao cls = new cls_LandingPage_QuangCao();
        ...
        else
        {
            ...
            if (Session["_id"].ToString() == "0")
            {
                ...
                if (cls.insert_Data(txtTitle.Text, txtLink.Text,image))
                {
                ...
                if (cls.Update_Data(Convert.ToInt32(Session["_id"].ToString()), txtTitle.Text,txtLink.Text, image))
            {
    ...
    {
        List<object> selectedKey = grvList.GetSelectedFieldValues(new string[] { "quangcao_id" });
        if (selectedKey.Count > 0)
        {
            foreach (var item in selectedKey)
            {
                cls_LandingPage_QuangCao cls = new cls_LandingPage_QuangCao();
                tbLandingPage_QuangCao checkImage = (from i in db.tbLandingPage_QuangCaos where i.quangcao_id == Convert.ToInt32(item) select i).SingleOrDefault();
