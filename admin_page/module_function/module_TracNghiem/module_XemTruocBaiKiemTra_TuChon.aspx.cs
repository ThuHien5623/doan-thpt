...
public partial class admin_page_module_function_module_TracNghiem_module_XemTruocBaiKiemTra_TracNghiem : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        int seed = rnd.Next();
        string listQuestions = (from bkt in db.tbTracNghiem_Tests
                                where bkt.test_id == test_id
                                select bkt.question_id).Single();
        var questionIds = listQuestions.Split(',').Select(int.Parse).ToList();
        //get tên bài, ds bài và tỉ lệ câu hỏi từng bài
        var getTracNghiem = from ch in db.tbTracNghiem_Questions
                            where questionIds.Contains(ch.question_id)
                            select new
                            {
                                ch.question_id,
