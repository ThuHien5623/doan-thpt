...
public partial class admin_page_module_function_module_TracNghiem_module_XemTruocBaiKiemTra_CauTrucMoi : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        rpCauHoi.DataBind();
        //câu hỏi đúng sai
        var getCauHoiDungSai = from bkt in db.tbTracNghiem_Tests
                               join ch in db.tbTracNghiem_Questions on bkt.test_id equals ch.baikiemtra_id
                               where bkt.test_id == test_id && ch.question_group == "part2"
                               select new
                               {
                                   ch.question_id,
        ...
        //get câu hỏi tự luận
        var getCauHoiTuLuan = from bkt in db.tbTracNghiem_Tests
                              join ch in db.tbTracNghiem_Questions on bkt.test_id equals ch.baikiemtra_id
                              join da in db.tbTracNghiem_Answers on ch.question_id equals da.question_id
                              where bkt.test_id == test_id && ch.question_group == "part3"
                              select new
                              {
                                  ch.question_id,
                                  da.answer_id,
