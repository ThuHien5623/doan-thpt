...
public partial class web_module_web_tracnghiem_vietnhatliencap_LamBaiLuyenTap : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Request.Cookies["PhuHuynhVietNhat"] != null)
        {
            ...
            rpCauHoi.DataSource = getDataDetails.OrderBy(x => (~(x.question_id & seed)) & (x.question_id | seed));
            rpCauHoi.DataBind();
            tongSoCau = getDataDetails.Count();
            Session["linktest"] = "";
            //if (RouteData.Values["type-test"].ToString() == "exams")
            //{
            var getBaiLuyenTap = (from blt in db.tbTracNghiem_BaiLuyenTaps
                                  ...
                                  select blt).First();
            title = getBaiLuyenTap.luyentap_name;
    ...
    protected void btnLuuKetQua_ServerClick(object sender, EventArgs e)
    {
        ...
        var checkHocSinh = (from hs in db.tbHocSinhs
                            ...
                            where hs.hocsinh_id == hocsinh_id
    ...
    {
        Response.Redirect("/danh-sach-bai-luyen-tap-10/3");
